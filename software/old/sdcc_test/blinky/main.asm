...
; This file was generated Thu Nov  6 11:41:45 2014
;--------------------------------------------------------
...
S_main__main	code
_main:
;	.line	42; main.c	init_port();
	CALL	_init_port
;	.line	43; main.c	init_timer();
	CALL	_init_timer
_00116_DS_:
;	.line	45; main.c	while(1) {
	BRA	_00116_DS_
	RETURN	
...
S_main__isr	code
_isr:
;	.line	49; main.c	void isr() __interrupt 1 {
	...
	MOVFF	FSR1L, FSR2L
;	.line	50; main.c	if (INTCONbits.TMR0IE && INTCONbits.TMR0IF) {
	BTFSS	_INTCONbits, 5
	BRA	_00134_DS_
	BTFSS	_INTCONbits, 2
	BRA	_00134_DS_
	BANKSEL	_timer_ticks
;	.line	52; main.c	timer_ticks++;
	INCF	_timer_ticks, F, B
	BANKSEL	_timer_ticks
;	.line	53; main.c	if (timer_ticks==20) {  // 80 ms
	MOVF	_timer_ticks, W, B
	XORLW	0x14
	BNZ	_00124_DS_
;	.line	54; main.c	LED = 0;            // LED OFF
	BCF	_LATAbits, 0
_00124_DS_:
	BANKSEL	_timer_ticks
;	.line	56; main.c	if (timer_ticks==40) {  // 80 ms
	MOVF	_timer_ticks, W, B
	XORLW	0x28
	BNZ	_00126_DS_
;	.line	57; main.c	LED = 1;            // LED ON
	BSF	_LATAbits, 0
_00126_DS_:
	BANKSEL	_timer_ticks
;	.line	59; main.c	if (timer_ticks==60) {  // 80 ms
	MOVF	_timer_ticks, W, B
	XORLW	0x3c
	BNZ	_00128_DS_
;	.line	60; main.c	LED = 0;            // LED OFF
	BCF	_LATAbits, 0
_00128_DS_:
	BANKSEL	_timer_ticks
;	.line	62; main.c	if (timer_ticks==250) { // 720 ms
	MOVF	_timer_ticks, W, B
	XORLW	0xfa
	BNZ	_00130_DS_
;	.line	63; main.c	LED = 1;            // LED ON
	BSF	_LATAbits, 0
	BANKSEL	_timer_ticks
;	.line	64; main.c	timer_ticks=0;
	CLRF	_timer_ticks, B
_00130_DS_:
;	.line	66; main.c	INTCONbits.TMR0IF = 0;
	BCF	_INTCONbits, 2
_00134_DS_:
	MOVFF	PREINC1, FSR2L
...
_init_timer:
;	.line	21; main.c	void init_timer(void) {
	MOVFF	FSR2L, POSTDEC1
	MOVFF	FSR1L, FSR2L
;	.line	25; main.c	T0CONbits.T0PS0=1;    //Prescaler is divide by 256
	BSF	_T0CONbits, 0
;	.line	26; main.c	T0CONbits.T0PS1=1;
	BSF	_T0CONbits, 1
;	.line	27; main.c	T0CONbits.T0PS2=1;
	BSF	_T0CONbits, 2
;	.line	29; main.c	T0CONbits.PSA=0;      //Timer Clock Source is from Prescaler
	BCF	_T0CONbits, 3
;	.line	30; main.c	T0CONbits.T0CS=0;     //Prescaler gets clock from FCPU (16MHz)
	BCF	_T0CONbits, 5
;	.line	32; main.c	T0CONbits.T08BIT=1;   //8 BIT MODE
	BSF	_T0CONbits, 6
;	.line	34; main.c	INTCONbits.TMR0IE=1;   //Enable TIMER0 Interrupt
	BSF	_INTCONbits, 5
;	.line	35; main.c	INTCONbits.PEIE=1;     //Enable Peripheral Interrupt
	BSF	_INTCONbits, 6
;	.line	36; main.c	INTCONbits.GIE=1;      //Enable INTs globally
	BSF	_INTCONbits, 7
;	.line	38; main.c	T0CONbits.TMR0ON=1;   //Now start the timer!
	BSF	_T0CONbits, 7
	MOVFF	PREINC1, FSR2L
	RETURN	
...
_init_port:
;	.line	16; main.c	void init_port(void) {
	MOVFF	FSR2L, POSTDEC1
	MOVFF	FSR1L, FSR2L
;	.line	17; main.c	ADCON1 = 0x0F;		// Default all pins to digital
	MOVLW	0x0f
	MOVWF	_ADCON1
;	.line	18; main.c	LED_TRIS = 0;
	BCF	_TRISAbits, 0
	MOVFF	PREINC1, FSR2L
	RETURN	
